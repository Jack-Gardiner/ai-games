...
bool SplendorState::is_valid_move(std::string move){
    ...
    if(move_type == "r"){
        ...
        return (buy_row < shopCards.size() && buy_col < shopCards[buy_row].size() && buy_col > -2);
    }
    if(move_type == "t"){
...
bool SplendorState::apply_move(std::string move){
    ...
    // example:
    // b 1,3 2,0,0,1,0 1
    // the above would use 3 gold as replacmenet for some gems, and buys the rare developement card on the rightmost side.
...
// TODO: display, get_board, and clone.
